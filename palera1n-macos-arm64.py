...
def rootless_boot_event():
    result = subprocess.run('./palera1n-macos-arm64 -v', shell=True, text=True)
rootless_boot.clicked.connect(rootless_boot_event)

def rootfull_boot_event():
    result = subprocess.run('./palera1n-macos-arm64 -f -v', shell=True, text=True)
rootfull_boot.clicked.connect(rootfull_boot_event)

def fakefs_event():
    result = subprocess.run('./palera1n-macos-arm64 -fc -v', shell=True, text=True)
fakefs.clicked.connect(fakefs_event)

def bindfs_event():
    result = subprocess.run('./palera1n-macos-arm64 -Bf -v', shell=True, text=True)
bindfs.clicked.connect(bindfs_event)

def force_revert_event():
    result = subprocess.run('./palera1n-macos-arm64 --force-revert -f -v', shell=True, text=True)
    output_field.append(result.stdout)
force_revert.clicked.connect(force_revert_event)

def pongo_shell_event():
    result = subprocess.run('./palera1n-macos-arm64 -p -v', shell=True, text=True)
...
def custom_args_event():
    ...
    if ok and args != '!cancel':
        commargs = './palera1n-macos-arm64 ' + args
